    ...
    <main className="min-w-[360px]">
      <section id="inicio" className="flex flex-col items-center h-screen justify-center">
        ...
        <div ref={buttonsRef} className="buttons flex gap-4 mt-16 mb-8">
          <Slide in={slide} direction="right" container={buttonsRef.current} {...(slide ? { timeout: 1000 } : {})}>
            <a href="#contact">
              <button className="action-btns">{t('contact')}</button>
            </a>
          </Slide>
          <Slide in={slide} direction="left" container={buttonsRef.current} {...(slide ? { timeout: 1000 } : {})}>
            <a target="_blank" href={i18n.language === 'es' ? 'https://drive.google.com/file/d/1rh595_SXvZgBsoaW1iR9ho6gUl-lYclc/view?usp=drive_link' : 'https://drive.google.com/file/d/1In4CRIfnMv504p4XGhJT_iUj8DrOPWxo/view?usp=drive_link'}>
              <button className="action-btns">{t('cv')}</button>
            </a>
          </Slide>
        </div>
        <div ref={socialRef}>
          <Slide in={slide} direction="up" container={socialRef.current} {...(slide ? { timeout: 1000 } : {})}>
            <div className="socialMedia flex gap-4">
              <a href="https://github.com/J-HernandezM" target="_blank">
                <GitHubIcon className="animate-bounce" fontSize="large" />
              </a>
              <a href="https://www.linkedin.com/in/juan-jose-hernandez-muñoz-9613821a2/" target="_blank">
                <LinkedInIcon className="animate-bounce" fontSize="large" />
              </a>
              <a href="mailto:juansw03@gmail.com" target="_blank">
                <MailIcon className="animate-bounce" fontSize="large" />
              </a>
            </div>
          </Slide>
        </div>
      ...
      <section id="about" className="flex flex-col items-center text-lg w-10/12 mx-auto lg:text-center gap-2 lg:gap-4">
        ...
        <div className="lg:w-2/3">
          <p className="text-pretty mb-2 lg:mb-4">{t('description.1')}</p>
          <p className="text-pretty mb-2 lg:mb-4">{t('description.2')}</p>
          <p className="text-pretty mb-2 lg:mb-4">{t('description.3')}</p>
          <p className="text-pretty">{t('description.4')}</p>
        </div>
      ...
      <section id="contact" className="mb-20 flex flex-col items-center gap-2 lg:gap-4 mt-4 lg:mt-8 w-10/12 mx-auto">
        ...
        <p>
          {t('footer.1')}
          <a href="https://www.linkedin.com/in/juan-jose-hernandez-muñoz-9613821a2/" target="_blank" className="font-semibold dark:text-light text-primary cursor-pointer">
            LinkedIn
          </a>
          {t('footer.2')}
          <a href="mailto:juansw03@gmail.com" target="_blank" className="font-semibold dark:text-light text-primary cursor-pointer">
            Gmail,
          </a>
          {t('footer.3')}
          <a href="https://github.com/J-HernandezM" target="_blank" className="font-semibold dark:text-light text-primary cursor-pointer">
            Github! :)
          </a>
        </p>
      </section>
